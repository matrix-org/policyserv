...
func TestHttpUnstableKeyDiscovery(t *testing.T) {
	t.Parallel()
...
func TestHttpKeyDiscovery(t *testing.T) {
	...
	for _, method := range []string{http.MethodGet, http.MethodOptions} {
		...
		res := httptest.NewRecorder()
		req := httptest.NewRequest(method, "/.well-known/matrix/policy_server", nil)
	...
	// Test wrong method too while we're here
	res := httptest.NewRecorder()
	req := httptest.NewRequest(http.MethodPost, "/.well-known/matrix/policy_server", nil)
	httpKeyDiscovery(server, res, req)
	assert.Equal(t, http.StatusMethodNotAllowed, res.Code)
	test.AssertApiError(t, res, "M_UNRECOGNIZED", "Method not allowed")
}
